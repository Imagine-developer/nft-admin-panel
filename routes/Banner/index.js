...
class Mail extends PureComponent {
  ...
  onSaveContact = async (data)=>{
    ...
    const res = await axios.post('https://desolate-inlet-76011.herokuapp.com/banner/upload', formData)
    const result = await axios.post('https://desolate-inlet-76011.herokuapp.com/banner', {title: data.title, text: data.text, url: data.url, imgUrl: res.data.img})
    console.log(result.data)
